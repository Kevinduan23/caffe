...
public:
  explicit FocalLossLayer(const LayerParameter &param)
      : LossLayer<Dtype>(param), has_ignore_label_(false), ignore_label_(0),
        normalization_(LossParameter_NormalizationMode_BATCH_SIZE),
        outer_num_(0), inner_num_(0), alpha_(0), gamma_(0) {}
...
protected:
  ...
  get_normalizer(LossParameter_NormalizationMode normalization_mode,
    ...
    // particular loss in a multi-task setup. The max prevents NaNs in that
    // case.
