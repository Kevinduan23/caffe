function net = get_net(varargin)
               % net = get_net(model_file, phase_name)
or
%
...
CHECK(ischar(model_file),

'model_file must be a string');

CHECK(ischar(phase_name),

'phase_name must be a string');
CHECK_FILE_EXIST(model_file);

CHECK (strcmp(phase_name,
