...
self.
net = caffe.Net(self.model_file, 'train');
%
also make
sure get_solver
runs
...
self.net.blobs('data').set_data(10 *
ones(self
...
10 *
ones(self
.net.blobs('data').shape, 'single'));
self.net.blobs('data').set_diff(-2 *
ones(self
.net.blobs('data').shape));
self.
verifyEqual(self
...
self.
verifyEqual(self
.net.blobs('data').shape, [6 5 4 3 2 1]);
self.net.blobs('data').
...
reshape();
        ...
        function
self = test_layer(self)
self.
verifyEqual(self
.net.params('conv', 1).shape, [2 2 2 11]);
self.
verifyEqual(self
.net.layers('conv').params(2).shape, 11);
self.
verifyEqual(self
.net.layers('conv').
