...
documentation and
usage on
Caffe's Matlab interface, please
%
refer to
the Caffe
Interface Tutorial
...
$ export
LD_LIBRARY_PATH =
/opt/intel/mkl/lib/intel64:/usr/local/cuda-5.5/lib64
%
$ export
LD_PRELOAD =
...
phase test(so that dropout isn
...
crops_data(:, :, :, n) =
im_data(i
:i+CROPPED_DIM-1, j:j+CROPPED_DIM-1, :);
crops_data(:, :, :, n+5) =
crops_data(end
:-1:1, :, :, n);
n = n + 1;
...
crops_data(:,:,:,5) = ...
im_data(center
:center+CROPPED_DIM-1,center:center+CROPPED_DIM-1,:);
crops_data(:,:,:,10) =
crops_data(end
:-1:1, :, :, 5);
