...
bool caffe::ReadFileToDatum(const string &filename,
  ...
  if (file.is_open()) {
    ...
    file.read(&buffer[0], size);
    file.close();
    COMPRESS_AND_SET(datum, buffer, compress)
    SET_LABELS(datum, labels)
    datum->set_encoded(true);
    return true;
  } else {
    return false;
...
bool caffe::ReadImageToDatum(const string &filename, const vector<float> &labels, const int height, const int width,
  ...
  if (cv_img.data) {
    if (encoding.size()) {
      if ((cv_img.channels() == 3) == is_color && !height && !width &&
          matchExt(filename, encoding))
        return ReadFileToDatum(filename, labels, datum, compress);
      std::vector<uchar> buf;
      cv::imencode("." + encoding, cv_img, buf);
      string data = std::string(reinterpret_cast<char *>(&buf[0]), buf.size());
      datum->set_encoded(true);
      COMPRESS_AND_SET(datum, data, compress)
      SET_LABELS(datum, labels)
      return true;
    }
    CVMatToDatum(cv_img, datum, compress);
    SET_LABELS(datum, labels)
    return true;
  } else {
    return false;
