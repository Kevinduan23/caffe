...
protected:
  GradientBasedSolverTest()
      : seed_(1701), num_(4), channels_(3), height_(10), width_(10),
        share_(false) {
    input_file_ = new string(ABS_TEST_DATA_DIR "/solver_data_list.txt");
  }

  ~GradientBasedSolverTest() { delete input_file_; }
  ...
  void
  ComputeLeastSquaresUpdate(const Dtype learning_rate, const Dtype weight_decay,
                            const Dtype momentum, const int num_iters,
    ...
    for (int i = 0; i <= D; ++i) {
      ...
      if (solver_->type() == string("SGD")) {
        ...
        const Dtype v = (i == D) ? history[1 + num_param_blobs]->cpu_data()[0]
                                 : history[0 + num_param_blobs]->cpu_data()[i];
  ...
  void CheckLeastSquaresUpdate(
    ...
    for (int i = 0; i < D; ++i) {
      ...
      const Dtype error_margin = std::max(
          kMinPrecision, kPrecision * std::min(fabs(expected_updated_weight),
    ...
    const Dtype error_margin = std::max(
        kMinPrecision, kPrecision * std::min(fabs(expected_updated_bias),
                                             fabs(solver_updated_bias)));
    ...
    if (solver_->type() == string("SGD")) {
      ...
      for (int i = 0; i < D; ++i) {
        ...
        const Dtype error_margin_hist = std::max(
            kMinPrecision, kPrecision * std::min(fabs(expected_history),
                                                 fabs(solver_history)));
      ...
      const Dtype error_margin_hist =
          std::max(kMinPrecision, kPrecision * std::min(fabs(expected_history),
                                                        fabs(solver_history)));
      EXPECT_NEAR(expected_history, solver_history, error_margin_hist);
    }
  }
  ...
  void CheckAccumulation(const Dtype kLearningRate, const Dtype kWeightDecay,
    ...
    for (int i = 0; i < D; ++i) {
      ...
      const Dtype error_margin =
          std::max(kMinPrecision, kPrecision * std::min(fabs(expected_param),
                                                        fabs(accum_param)));
  ...
  void TestSnapshot(const Dtype learning_rate = 1.0,
    ...
    string snapshot_name =
        RunLeastSquaresSolver(learning_rate, weight_decay, momentum, num_iters,
                              kIterSize, kDevices, snapshot);
