...
void SmoothL1LossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
  ...
  if (has_weght_) {
    caffe_mul(count, bottom[2]->cpu_data(), diff_.cpu_data(),
              diff_.mutable_cpu_data());
  }
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (abs_val < 1.0)
      error_data[i] = 0.5 * pow(abs_val, 2);
    else
...
void SmoothL1LossLayer<Dtype>::Backward_cpu(
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (fabs(val) < 1.0)
      ...
      diff_data[i] = val < -1.0 ? -1.0 : 1.0;
  }
  for (int j = 0; j < 2; ++j) {
    if (propagate_down[j]) {
      const Dtype sign = j == 0 ? 1.0 : -1.0;
      const Dtype alpha = sign * top[0]->cpu_diff()[0] / bottom[j]->shape(0);
      caffe_cpu_axpby(bottom[j]->count(), alpha, diff_.cpu_data(), Dtype(0),
