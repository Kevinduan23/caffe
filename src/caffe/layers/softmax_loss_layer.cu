    ...
    if (has_ignore_label_ && label_value == ignore_label_) {
      ...
      if (use_label_smooth && (label_smooth_factor > 0.0F)) {
        for (int c = 0; c < num_classes; c++) {
          float coeff = (c == label_value)
                            ? (1.0F - label_smooth_factor)
                            : (label_smooth_factor / float(num_classes));
          loss[index] -=
              coeff * log(max(prob_data[n * dim + c * spatial_dim + s],
                              Dtype(FLT_MIN)));
        }
      } else {
        loss[index] =
...
void SoftmaxWithLossLayer<Dtype>::Forward_gpu(
  ...
  SoftmaxLossForwardGPU<Dtype>
      <<<CAFFE_GET_BLOCKS(nthreads), CAFFE_CUDA_NUM_THREADS>>>(
          ...
          has_ignore_label_, ignore_label_, counts, use_label_smooth_,
          label_smooth_factor_, num_classes_);
  Dtype loss;
  ...
  top[0]->mutable_cpu_data()[0] =
      loss / get_normalizer(normalization_, valid_count);
  if (top.size() == 2) {
...
void SoftmaxWithLossLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    const Dtype loss_weight =
        top[0]->cpu_diff()[0] / get_normalizer(normalization_, valid_count);
