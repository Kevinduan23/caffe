...
namespace caffe {
  ...
  __global__ void SmoothL1Forward(const int n, const Dtype *in, Dtype *out) {
    ...
    CUDA_KERNEL_LOOP(index, n)
    {
      ...
      if (abs_val < 1.0) out[index] = 0.5 * pow(val, 2);
      else out[index] = abs_val - 0.5;
    }
  }

  template<typename Dtype>
  void SmoothL1LossLayer<Dtype>::Forward_gpu(const std::vector<caffe::Blob<Dtype> *> &bottom,
    ...
    SmoothL1LossLayer<Dtype> << < CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >> > (
