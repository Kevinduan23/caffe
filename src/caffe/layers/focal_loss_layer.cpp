...
namespace caffe {
  ...
  template<typename Dtype>
  void FocalLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top, const vector<bool> &propagate_down,
    ...
    if (propagate_down[0]) {
      ...
      for (int i = 0; i < outer_num_; ++i) {
        ...
        for (int j = 0; j < inner_num_; ++j) {
          ...
          if (j == label_value) {
            focal_diff = alpha_ * gamma_ * log(std::max(pt, Dtype(FLT_MIN))) * pow(1.0 - pt, gamma_ - 1)
                         - (alpha_ * pow(1.0 - pt, gamma_)) / std::max(pt, Dtype(FLT_MIN));
          } else {
            focal_diff = -((alpha_ * pow(pt, gamma_)) / (pt - 1.0))
                         - alpha_ * pow(pt, gamma_ - 1) * gamma_ * log(std::max(Dtype(1.0 - pt), Dtype(FLT_MIN)));
          }
